...
import { findHighestDefaultLayout } from '../shared/assets/layoutDefaults.js';
...
export function loadDefaultLayout(assetsRoot: string): Record<string, unknown> | null {
  ...
  try {
    const found = findHighestDefaultLayout(assetsDir);
    if (!found) {
    ...
    if (found.revision > 0 && !layout[LAYOUT_REVISION_KEY]) {
      layout[LAYOUT_REVISION_KEY] = found.revision;
    }
    console.log(
      `[AssetLoader] Loaded default layout (${layout.cols}×${layout.rows}, revision ${layout[LAYOUT_REVISION_KEY] ?? 0}) from ${path.basename(found.path)}`,
    );
    return layout;
  } catch (err) {
    console.error(

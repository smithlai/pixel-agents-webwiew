...
interface DecodedPng {
  ...
  /** Source canvas the pixels were read from. Reuse via drawImage() to avoid
   *  a second createImageBitmap round-trip in the high-res frame splitter. */
...
async function decodePng(url: string): Promise<DecodedPng> {
  ...
  return { width: canvas.width, height: canvas.height, data: imageData.data, source: canvas };
}
  ...
  for (let i = 0; i < index.characters.length; i++) {
    ...
    for (let dirIdx = 0; dirIdx < CHARACTER_DIRECTIONS.length; dirIdx++) {
      ...
      for (let frame = 0; frame < CHAR_FRAMES_PER_ROW; frame++) {
        ...
        fc.width = srcFrameW;
        fc.height = srcFrameH;
        fc.getContext('2d')!.drawImage(png.source, sx, sy, srcFrameW, srcFrameH, 0, 0, srcFrameW, srcFrameH);
        dirFrames.push(fc);
      }
...
async function decodeCharactersFromPng(
  ...
  for (const relPath of index.characters) {
    ...
    if (isHighRes) {
      // 高解析度素材：直接從 decodePng() 的 source canvas 切幀，不再 round-trip
      // 一次 createImageBitmap。瀏覽器 canvas 本身就處理好了 premultiplied alpha。
      const srcFrameW = Math.floor(png.width / CHAR_FRAMES_PER_ROW);
      const srcFrameH = Math.floor(png.height / CHARACTER_DIRECTIONS.length);

      // 預切幀：每幀獨立 canvas，直接從 source canvas 切出正確像素邊界
      const frameCanvases: HTMLCanvasElement[][] = [];
      ...
      for (let dirIdx = 0; dirIdx < CHARACTER_DIRECTIONS.length; dirIdx++) {
        ...
        for (let frame = 0; frame < CHAR_FRAMES_PER_ROW; frame++) {
          ...
          smallCtx.drawImage(png.source, sx, sy, srcFrameW, srcFrameH, 0, 0, CHAR_FRAME_W, CHAR_FRAME_H);
          spriteFrames.push(readSprite(
            {
              width: CHAR_FRAME_W,
              height: CHAR_FRAME_H,
              data: smallCtx.getImageData(0, 0, CHAR_FRAME_W, CHAR_FRAME_H).data,
              source: smallCanvas,
            },

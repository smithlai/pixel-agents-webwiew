...
在 `webview-ui/` 複製 `.env.example` 為 `.env.local`（已加入 .gitignore，不會上傳）：
...
之後直接 `npm run dev` 即可，不需要額外設定 shell 環境變數。
...
未設定時，Goose 事件串流停用，自動 fallback 到 mock 模式，console 會提示：
```
[GooseOffice] 提示：複製 .env.example 為 .env.local 並填入路徑即可啟用
```

**模擬腳本測試**（端對端驗證：檔案→watcher→WebSocket→瀏覽器）：

```bash
# 另開一個終端，目錄與 GOOSE_WATCH_DIR 一致
...
| DroidClaw 動態 spawn | Tester 派出 DroidClaw 時以 Matrix 光柱特效動態出現，任務完成後消散 |
| Boss 角色 + 指令輸入框 | 畫面下方可輸入指令觸發 Boss 動畫，PM 自動前往 Boss 桌前匯報 |
| 空間行為動線 | Agent 收到任務→匯報上司→走回工位工作→完成→走去休息位 |
| 瀏覽器 Layout 持久化 | F5 重載後佈局不再重置，透過 localStorage 保存 |
